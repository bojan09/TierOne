...
function addCourse(course: Omit<Course,'moduleIds'|'track'|'difficulty'> & { difficulty?: Course['difficulty'] }, moduleId: string, seeds: {id:string;slug:string;title:string;xp:number;minutes:number;hasQuiz?:boolean}[]) {
  ...
  const built: Lesson[] = seeds.map((s,i)=>({ id:s.id, slug:s.slug, title:s.title, courseId:course.id, moduleId, order:i+1, xp:s.xp, track:'scripting', difficulty:level, estimatedMinutes:s.minutes, lockRule:{type:'none'} as LockRule, hasQuiz:Boolean(s.hasQuiz) }));
  lessons.push(...built);
